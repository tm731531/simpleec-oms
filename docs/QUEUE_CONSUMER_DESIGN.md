...
### 報表生成策略（task.backend） — 時區感知 + 負載分散

```
⚠️  關鍵：避免 Thundering Herd 問題 — 不能所有報表都在同一分鐘觸發！

Scheduler 根據「商家 ID 或客戶 ID」的雜湊計算偏移，分散觸發時間：

5 分鐘間隔（小時報表）：
  - 商家 M001 → 偏移 1 分鐘  → 01, 06, 11, 16, 21, 26, 31, 36, 41, 46, 51, 56
  - 商家 M002 → 偏移 6 分鐘  → 06, 11, 16, 21, 26, 31, 36, 41, 46, 51, 56, 01
  - 商家 M003 → 偏移 11 分鐘 → 11, 16, 21, 26, 31, 36, 41, 46, 51, 56, 01, 06
  （hash(merchantId) % 5 決定偏移，避免競爭）

10 分鐘間隔（主動狀態更新）：
  - 商家 M001 → 偏移 12 分鐘 → 12, 22, 32, 42, 52, 02, 12
  - 商家 M002 → 偏移 22 分鐘 → 22, 32, 42, 52, 02, 12, 22
  - 商家 M003 → 偏移 32 分鐘 → 32, 42, 52, 02, 12, 22, 32
  （hash(merchantId) % 10 決定偏移）

30 分鐘和整點（日報）：
  - 所有商家統一：00 分和 30 分（無偏移，因為日報頻率低）
...
範例：
  ...
  customer A (M001, UTC+8):
    → 本地時間 2026-02-14T00:06:00+08:00
    → 偵測到跨 0 點 ✓ → 同時觸發「日報」

  customer B (M002, UTC-5):
